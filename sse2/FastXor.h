...
public:
	...
	static inline void Xor(std::vector<uint8_t>& data, const std::vector<uint8_t>& key) {
		Xor(data.data(), data.size(), key.data(), key.size());
	}

	static inline void Xor(std::string& data, const std::vector<uint8_t>& key) {
		Xor((uint8_t*)data.data(), data.size(), key.data(), key.size());
	}
	
	static inline void Xor(std::string& data, const std::string& key) {
		Xor((uint8_t*)data.data(), data.size(), (const uint8_t*)key.data(), key.size());
	}

	static inline void Xor(std::vector<uint8_t>& data, const std::string& key) {
